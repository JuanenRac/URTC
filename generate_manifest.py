...
def find_versioned_bin(fw_dir, prefix):
    """build_firmware.sh/.bat embed each binary's own version in its
    filename (e.g. URTC_MAIN_BOOTLOADER_v0.0.0.bin - same
    <PREFIX>_v<MAJOR>.<MINOR>.<PATCH> convention as sibling repo HYDRA-UMC's
    own build_firmware.sh) - glob for it rather than reconstructing the
    exact name here, so this script can't silently drift out of sync with
    those scripts' own naming convention."""
    matches = sorted(glob.glob(os.path.join(fw_dir, f"{prefix}_v*.bin")))
    if not matches:
        raise SystemExit(
            f"generate_manifest.py: no {prefix}_v*.bin found in {fw_dir} - "
...
def bin_info(bin_path):
    with open(bin_path, "rb") as f:
        data = f.read()
    crc = zlib.crc32(data) & 0xFFFFFFFF
    return {"filename": os.path.basename(bin_path), "size_bytes": len(data), "crc32": f"0x{crc:08X}"}
...
def component_files(fw_dir, prefix):
    """Locates PREFIX_v*.bin in firmware/ via find_versioned_bin(), then
    builds the bin/hex/elf "files" block every component entry below needs -
    one place for this so the 4 entries in main() below stay short."""
    bin_path = find_versioned_bin(fw_dir, prefix)
    return {
    ...
    manifest = {
        ...
        "components": {
            "main_bootloader": {
                ...
                "files": component_files(fw_dir, "URTC_MAIN_BOOTLOADER"),
            },
            "main_application": {
                ...
                "files": component_files(fw_dir, "URTC_MAIN_FIRMWARE"),
            },
            "slave_bootloader": {
                ...
                "files": component_files(fw_dir, "URTC_SLAVE_BOOTLOADER"),
            },
            "slave_application": {
                ...
                "files": component_files(fw_dir, "URTC_SLAVE_FIRMWARE"),
            },
        },
    }
