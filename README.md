...
0x08008000 ├─────────────────────────────────┤
           ...
           │                                   │  URTC_MAIN_FIRMWARE_v0.2.7.* — the actual
...
**Versioning policy:** all 4 components (2 application firmwares, 2 bootloaders - `FIRMWARE_VERSION_MAJOR`/`MINOR`/`PATCH` and `BOOTLOADER_VERSION_MAJOR`/`MINOR`/`PATCH`) are **incremental** - every real build automatically bumps that component's own `PATCH` by 1 (`bump_version.py` at the repo root, called by `build_firmware.sh`/`.bat` right before compiling each component), carrying into `MINOR` (then `MAJOR`) once `PATCH` would pass 9, the same base-10 "odometer" rule a car's own odometer uses - e.g. `0.1.7` → `0.1.8` → `0.1.9` → `0.2.0`, never `0.1.10`. Each bootloader also keeps its own copy of the matching application's `FIRMWARE_VERSION_*`, kept in sync automatically by the same bump. See [`CHANGELOG.md`](CHANGELOG.md) at the repo root for the current version of all 4 components at a glance, and [`VERSION_CHECKLIST.txt`](VERSION_CHECKLIST.txt) for the full per-track mechanics.
...
│   ├── URTC_MAIN_FIRMWARE_v0.2.7.bin    Application bin compiled, flash to 0x08008000
│   ├── URTC_MAIN_FIRMWARE_v0.2.7.elf    Application elf compiled, flash to 0x08008000
│   ├── URTC_MAIN_FIRMWARE_v0.2.7.hex    Application HEX compiled, flash to 0x08008000 (address baked in)
...
**Child Projects** — each one is a desktop or browser tool built specifically to flash, test, or drive this firmware
- **[URTC-FLASHER](https://github.com/JuanenRac/URTC-FLASHER)** — desktop GUI flashing tool for URTC boards, CAN-OTA plus full-chip SWD/JTAG; flashes this exact firmware.
- **[URTC-TESTER](https://github.com/JuanenRac/URTC-TESTER)** — desktop live CAN-bus diagnostic tool for URTC boards, one panel per tool profile; one panel per this firmware's own 25 tool profiles.
- **[URTC-WEB-STUDIO](https://github.com/JuanenRac/URTC-WEB-STUDIO)** — browser-based alternative to URTC-TESTER via the Web Serial API, no local install needed; the same CAN protocol as the 2 desktop tools above.

**Directly Related**
- **[HYDRA-UMC](https://github.com/JuanenRac/HYDRA-UMC)** — the physical robot-arm motherboard: CM5 host + dual-core STM32H745, orchestrating up to 8 tool arms over CAN-OTA/SPI-OTA; its own real FDCAN2 relay tunnel is the wire this firmware's tool head sits on.
- **[HYDRA-UMC-SDK](https://github.com/JuanenRac/HYDRA-UMC-SDK)** — the shared JSON-Schema contract and safety-gate boundary every bridge validates its commands against; also covers services, UIs, CM5 adapters, and this firmware.
- **[URTC-SMART-RACK](https://github.com/JuanenRac/URTC-SMART-RACK)** — firmware for a board-mounting rack with real tool-ID decoding and Smart Idle pre-heating logic; shares this firmware's own tool ecosystem and CAN bus.
- **[URTC-VISION-TOOL](https://github.com/JuanenRac/URTC-VISION-TOOL)** — firmware plus a real Python vision companion for a thermal/RGB inspection tool head; shares this firmware's own tool ecosystem and CAN bus.
- **[HYDRA-UMC-DETECTION-HEF](https://github.com/JuanenRac/HYDRA-UMC-DETECTION-HEF)** — real compiled-model registry with Hailo-architecture/checksum safe-load verification; provides the visual recognition behind this firmware's own tools.
...
3. The **documentation** (this README and its own translations - `README_spa.md`, `README_ita.md`, `README_fra.md`, `README_deu.md`, `README_zho.md`, `README_jpn.md` - plus the reference files under `./docs`) is available under **Creative Commons Attribution-ShareAlike 4.0 International (CC BY-SA 4.0)**. Full text at https://creativecommons.org/licenses/by-sa/4.0/.
