...
**Versioning policy:** the 2 application firmware version numbers (`FIRMWARE_VERSION_MAJOR`/`MINOR`, main board and expansion slave) are **static** - they only change when a human edits them by hand. The 2 bootloader version numbers (`BOOTLOADER_VERSION_MAJOR`/`MINOR`/`PATCH`) are **incremental** - every real build automatically bumps `PATCH` by 1 (`bump_bootloader_version.py` at the repo root, called by `build_firmware.sh`/`.bat` right before compiling each bootloader), carrying into `MINOR` (then `MAJOR`) once `PATCH` would pass 9, the same base-10 "odometer" rule a car's own odometer uses - e.g. `1.1.7` → `1.1.8` → `1.1.9` → `1.2.0`, never `1.1.10`. See [`CHANGELOG.md`](CHANGELOG.md) at the repo root for the current version of all 4 components at a glance, and [`VERSION_CHECKLIST.txt`](VERSION_CHECKLIST.txt) for the full per-track mechanics.
