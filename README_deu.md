...
**Versionierungsrichtlinie:** Die 2 Versionsnummern der Anwendungsfirmware (`FIRMWARE_VERSION_MAJOR`/`MINOR`, Hauptplatine und Erweiterungs-Slave) sind **statisch** - sie ändern sich nur, wenn ein Mensch sie manuell bearbeitet. Die 2 Bootloader-Versionsnummern (`BOOTLOADER_VERSION_MAJOR`/`MINOR`/`PATCH`) sind **inkrementell** - jeder reale Build erhöht `PATCH` automatisch um 1 (`bump_bootloader_version.py` im Repository-Root, aufgerufen von `build_firmware.sh`/`.bat` unmittelbar vor dem Kompilieren jedes Bootloaders), mit Übertrag auf `MINOR` (dann `MAJOR`), sobald `PATCH` 9 überschreiten würde - dieselbe Zehnerbasis-Regel wie bei einem echten Kilometerzähler, z. B. `1.1.7` → `1.1.8` → `1.1.9` → `1.2.0`, niemals `1.1.10`. Siehe [`CHANGELOG.md`](CHANGELOG.md) im Repository-Root für den aktuellen Stand aller 4 Komponenten auf einen Blick, und [`VERSION_CHECKLIST.txt`](VERSION_CHECKLIST.txt) für die vollständige Mechanik je Track.
...
├── firmware/
│   ├── URTC_BOOTLOADER.bin       Bootloader kompiliert, flashen auf 0x08000000
│   ├── URTC_BOOTLOADER.elf       Bootloader kompiliert, flashen auf 0x08000000
│   ├── URTC_BOOTLOADER.hex       Bootloader kompiliert, flashen auf 0x08000000 (Adresse eingebettet)
│   ├── URTC_V1.1_F303CC.bin      Anwendungs-bin kompiliert, flashen auf 0x08008000
│   ├── URTC_V1.1_F303CC.elf      Anwendungs-elf kompiliert, flashen auf 0x08008000
│   ├── URTC_V1.1_F303CC.hex      Anwendungs-HEX kompiliert, flashen auf 0x08008000 (Adresse eingebettet)
│   ├── URTC_SLAVE_BOOTLOADER.{bin,elf,hex}  Eigener Bootloader des Erweiterungs-Slave, flashen auf 0x08000000
│   │                             auf dem STM32F303CBT6 (nur advanced-Erweiterungsplatinen)
│   ├── URTC_SLAVE_APP.{bin,elf,hex}  Eigene Anwendung des Erweiterungs-Slave, flashen auf 0x08005000
...
Dieses Projekt ist Teil eines größeren Robotik-Ökosystems desselben Autors (JuanenRac / Electro Hobby 3D). Gut zu wissen, da eine Anfrage tatsächlich eines dieser Projekte betreffen könnte, statt dieses Repositorys:

**HYDRA-UMC-Plattform** — die Multi-Roboter-Mikrofabrikzelle
- **[HYDRA-UMC](https://github.com/JuanenRac/HYDRA-UMC)** — die Hauptplatine selbst: Raspberry Pi CM5 Host + Dual-Core-STM32H745-Echtzeit-Koprozessor, orchestriert bis zu 8 verteilte Roboterarme über CAN-OTA/SPI-OTA. Eigene Hardware + Firmware, GPL-3.0/CERN-OHL-S v2/CC BY-SA 4.0.
- **[HYDRA-UMC STUDIO](https://github.com/JuanenRac/HYDRA-UMC-STUDIO)** — webbasiertes Steuerungs-Dashboard für HYDRA-UMC: Multi-Roboter-3D-Visualisierung, Kinematik-/Trajektorien-Aufzeichnung, CAN-OTA-Flashen und -Testen für die gesamte Plattform. React + Vite + Three.js.
- **[HYDRA-UMC-ANDROID-CONTROL](https://github.com/JuanenRac/HYDRA-UMC-ANDROID-CONTROL)** — Android-Steuerungsapp für HYDRA-UMC über Wi-Fi/Bluetooth. Echte, funktionierende App - vollständiger Fernsteuerungs-Funktionsumfang, JWT-Authentifizierung, verschlüsselte Zugangsdatenspeicherung.
- **[HYDRA-UMC-IOS-CONTROL](https://github.com/JuanenRac/HYDRA-UMC-IOS-CONTROL)** — iOS/iPadOS-Steuerungsapp für HYDRA-UMC über Wi-Fi, gebaut in Flutter (plattformübergreifend, unter Windows ohne Mac überprüfbar; die endgültige `.ipa`-Paketierung benötigt noch Xcode). Echte, funktionierende App - derselbe Funktionsumfang wie die Android-App.
- **[HYDRA-UMC-SUITE](https://github.com/JuanenRac/HYDRA-UMC-SUITE)** — Desktop-(Python/PySide6)-Schwarm-Kommandozentrale: Multi-Controller-Netzwerkerkennung, live-bidirektionale Synchronisation, echtes 3D-Roboter-Viewport, Photoshop-artiger andockbarer Arbeitsbereich. Echt und funktionierend, kein Platzhalter.
- **[HYDRA-UMC-EDITOR-URDF](https://github.com/JuanenRac/HYDRA-UMC-EDITOR-URDF)** — Desktop-(Python/PySide6)-grafischer URDF-Ersteller/-Editor für den eigenen Modellkatalog dieses Projekts: zieht Quelldateien von GitHub oder einem lokalen Ordner, validiert DOF-Machbarkeit, bearbeitet Farbe/Skalierung/Kinematik mit Live-3D-Vorschau, und schiebt das fertige Ergebnis zu einem laufenden STUDIO-Server. Echt und funktionierend, kein Platzhalter.
...
- **URTC** *(dieses Repository)* — Universal Robot Tool Controller: STM32F303-basierter CAN-Bus-Werkzeugkopf-Controller, 25 vollständig implementierte Werkzeugprofile, CAN-OTA-Firmware-Update.
- **[URTC Flasher](https://github.com/JuanenRac/URTC-FLASHER)** — Desktop-CAN-OTA- + Full-Chip-SWD/JTAG-Flash-Tool für URTC-Platinen (Windows/Linux).
- **[URTC Tester](https://github.com/JuanenRac/URTC-TESTER)** — Desktop-Live-CAN-Bus-Diagnosetool für URTC-Platinen, ein Panel pro Werkzeugprofil (Windows/Linux).
...
3. Die **Dokumentation** (dieses README und dessen eigene Übersetzungen - `README_spa.md`, `README_ita.md`, `README_fra.md`, `README_deu.md` - plus die Referenzdateien unter `./docs`) ist verfügbar unter **Creative Commons Attribution-ShareAlike 4.0 International (CC BY-SA 4.0)**. Vollständiger Text unter https://creativecommons.org/licenses/by-sa/4.0/.
