...
CAN protocol (every ID, byte layout, and DLC), see `docs/CANBUS.TXT` — this
document explains *why* the system is built the way it is; `docs/CANBUS.TXT`
is the byte-for-byte reference. For pin-by-pin detail, see `docs/PINOUT.TXT`
and `docs/PINOUT_CONNECTORS.TXT`.
...
`tools/flasher/V1.0/urtc_flasher.py`'s own `BOOTLOADER_FLASH_ADDR` /
...
the safety-ceiling comment in `STM32F303CC.C` itself for the full
...
## 8. Partitioned build (`partitioned/`)

Everything above describes one monolithic source file. The exact same
firmware also exists split across 60 `.c`/`.h` files by subsystem, in
`partitioned/` right next to the monolithic source (and the bootloader's
own 9-file split lives in `boot/partitioned/`). Both forms are
maintained side by side going forward - a change to one gets mirrored to
the other - rather than one being generated from the other or one being
deprecated in favor of the other.

**Why both exist:** the monolithic file is simpler to grep through and
diff as a whole; the partitioned form reads more like a typical embedded
project of this size usually looks, with each subsystem's code and its
own header living together, and room to grow into it further as more
tool profiles are added (see the next paragraph).

**Layout** - one file (or small group of files) per subsystem. Every
`.c` file below has a matching `.h` of the same name unless noted
otherwise; only the `.c` is listed where that's the case, to keep this
table one row per subsystem rather than two.
...
| `firmware_expansion_spi.c` | `CONN_EXPANSION`'s own bit-banged SPI bus. |
| `firmware_expansion_i2c.c` | `CONN_EXPANSION`'s own bit-banged I2C bus - see section 6 above for why this is bit-banged rather than a hardware peripheral. |
...
| `firmware_control_thermal.c` | Soldering iron and 3D hotend PID control loops, including the independent safety ceilings. |
| `firmware_control_sensors.c` | Vacuum pickup and endstop sensor reading. |
...
| `firmware_can_motion.c` | Shared motion-tool CAN handler (`0x120`) - paste/liquid dispenser, screwdriver, both grippers. |
...
| `firmware_interrupts_timer.c` | Step-pulse generation ISR (TIM3) and its own concurrency-safe `steps_remaining` handling. |
...
place in flash - see section 1's flash layout table) one level up,
alongside the monolithic `STM32F303CC.C` and this README.

**Adding a new tool profile** (16 more are planned, for 28 total) means
writing that tool's own `firmware_can_<toolname>.c/.h` with a
`Handle_CAN_<ToolName>(void)` function, then adding one `case` to the
switch in `firmware_can_dispatch.c` calling it - nothing else in that
file changes, and no other module needs to know the new tool exists
unless it specifically interacts with it.

**Verified equivalent, not just "should behave the same":** both forms
are built and their outputs compared directly - `.rodata` (every
constant data table: the font, tool icons, splash frames, etc.) comes
out byte-for-byte identical between the two, and `.text` differs by well
under 1%, fully explained by the monolithic compiler's own extra
cross-function inlining opportunities within a single translation unit
(confirmed by checking that the "missing" functions don't appear as
separate symbols at all in the monolithic build, rather than actually
being absent).
