...
// temperatura en tiempo real en el OLED" -
// SONNET/AUDITORIA_COMPLETA_44_PROYECTOS.txt) - confined to ONE 8px-tall
...
// text/endstop/progress-bar below - confirmed unused by grepping every
// OLED_SetCursor(4, ...) call in this file: there are none). A coarse
// trend line, not a labeled axis chart - 8 vertical pixels is only ever
// going to show "rising/falling/flat", which is exactly what a glance at
// this screen actually needs (the precise number is already right there
// as text on page 5).
//
// Call this every render tick with the CURRENT instantaneous value
// (Render_ToolScreen already does, at its own 150ms cadence) - internally
// rate-limited to one real history sample every ~2s via its own static
// state, so the visible trend spans roughly a minute (SPARK_SAMPLES x
// SPARK_SAMPLE_PERIOD_MS) instead of being dominated by noise between
// consecutive 150ms render calls.
#define SPARK_SAMPLES 32
#define SPARK_SAMPLE_PERIOD_MS 2000
...
void OLED_DrawTempSparkline(uint8_t page, uint8_t col_start, uint8_t width, uint16_t current_value, uint16_t min_value, uint16_t max_value) {
    static uint16_t history[SPARK_SAMPLES] = {0};
    static uint8_t sample_count = 0;   // how many of `history` are real samples yet (ramps up to SPARK_SAMPLES once, then stays there)
    static uint8_t write_index = 0;    // next slot to overwrite - circular
    static uint32_t last_sample_tick = 0;

    uint32_t now = HAL_GetTick();
    if (now - last_sample_tick >= SPARK_SAMPLE_PERIOD_MS || sample_count == 0) {
        last_sample_tick = now;
        history[write_index] = current_value;
        write_index = (write_index + 1) % SPARK_SAMPLES;
        if (sample_count < SPARK_SAMPLES) sample_count++;
    }

    if (width == 0 || max_value <= min_value) return;
    if (width > 128) width = 128;
    if (col_start < 128 && (uint16_t)col_start + width > 128) {
        width = 128 - col_start;
    }

    uint8_t col_buffer[128];
    for (uint8_t col = 0; col < width; col++) {
        // Oldest sample at the left edge, newest at the right - same
        // left-to-right "time flows forward" convention as URTC-TESTER's
        // own live graph (HYDRA-UMC-SUITE/URTC-TESTER's _create_live_graph),
        // so an operator reading either one doesn't have to remember two
        // different conventions.
        uint8_t sample_idx = (col * sample_count) / (width > 1 ? width : 1);
        if (sample_idx >= sample_count) sample_idx = sample_count > 0 ? sample_count - 1 : 0;
        // history is circular - the OLDEST valid sample is at write_index
        // once the buffer has wrapped (sample_count == SPARK_SAMPLES),
        // otherwise the oldest is simply history[0] (nothing's wrapped yet).
        uint8_t real_idx;
        if (sample_count < SPARK_SAMPLES) {
            real_idx = sample_idx;
        } else {
            real_idx = (write_index + sample_idx) % SPARK_SAMPLES;
        }

        uint16_t v = (sample_count > 0) ? history[real_idx] : current_value;
        if (v < min_value) v = min_value;
        if (v > max_value) v = max_value;
        // Map value to a row 0-7 within this one page (7 = bottom/coldest,
        // 0 = top/hottest - a taller bar-like column reads as "more" the
        // same way OLED_DrawHorizontalBar's own filled columns do).
        uint8_t row_from_bottom = (uint8_t)(((uint32_t)(v - min_value) * 7) / (max_value - min_value));
        // Bits are LSB-first from the page's own top row (SSD1306 page-
        // addressing convention, same bit order OLED_DrawHorizontalBar's
        // own 0x7E/0x5E/0x42 patterns above already rely on) - a filled
        // column from the bottom up to row_from_bottom.
        uint8_t byte = 0;
        for (uint8_t bit = 7; bit >= (7 - row_from_bottom); bit--) {
            byte |= (1 << bit);
            if (bit == 0) break; // uint8_t bit would wrap past 0 instead of going negative - a real bug this guards against
        }
        col_buffer[col] = byte;
    }

    OLED_SetCursor(page, col_start);
    OLED_WriteData(col_buffer, width);
}

// 16x16 monochrome icon table, one per each of the 25 currently-assigned
// tools.
...
static const uint8_t ToolIcons[25][4][2][16] = {
    ...
    // 12 TOOL_SMT_PICKPLACE
    {
       {{0x00,0x00,0x00,0x00,0x00,0xE0,0xE0,0xFF,0xE0,0xE0,0xE0,0x00,0x00,0x00,0x00,0x00}, {0x00,0x00,0x00,0x38,0x28,0x28,0x28,0x28,0x28,0x28,0x28,0x28,0x38,0x00,0x00,0x00}},
       {{0x00,0x00,0x00,0x00,0x00,0xC0,0xC0,0xFF,0xC0,0xC0,0xC0,0x00,0x00,0x00,0x00,0x00}, {0x00,0x00,0x00,0x38,0x28,0x29,0x29,0x29,0x29,0x29,0x29,0x28,0x38,0x00,0x00,0x00}},
       {{0x00,0x00,0x00,0x00,0x00,0x80,0x80,0xFF,0x80,0x80,0x80,0x00,0x00,0x00,0x00,0x00}, {0x00,0x00,0x00,0x38,0x28,0x2B,0x2B,0x2B,0x2B,0x2B,0x2B,0x28,0x38,0x00,0x00,0x00}},
       {{0x00,0x00,0x00,0x00,0x00,0xC0,0xC0,0xFF,0xC0,0xC0,0xC0,0x00,0x00,0x00,0x00,0x00}, {0x00,0x00,0x00,0x38,0x28,0x29,0x29,0x29,0x29,0x29,0x29,0x28,0x38,0x00,0x00,0x00}}
    },
    // 13 TOOL_ELECTROMAGNET
    {
       {{0x00,0x00,0x80,0x80,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x80,0x80,0x00}, {0x00,0x70,0x7F,0x7F,0x7C,0x18,0x30,0x30,0x30,0x30,0x18,0x7C,0x7F,0x7F,0x7F,0x00}},
       {{0x00,0x00,0x80,0x80,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x80,0x80,0x00}, {0x00,0x70,0x7F,0x7F,0x7C,0xF8,0x30,0x30,0x30,0x30,0xF8,0x7C,0x7F,0x7F,0x7F,0x00}},
       {{0x00,0x00,0x80,0x80,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x80,0x80,0x00}, {0x00,0x70,0x7F,0x7F,0x7C,0xF8,0x30,0x30,0x30,0x30,0xF8,0x7C,0x7F,0x7F,0x7F,0x00}},
       {{0x00,0x00,0x80,0x80,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x80,0x80,0x00}, {0x00,0x70,0x7F,0x7F,0x7C,0x18,0x30,0x30,0x30,0x30,0x18,0x7C,0x7F,0x7F,0x7F,0x00}}
    },
    // 14 TOOL_SPOT_WELDER
    {
       {{0x00,0x00,0x00,0x01,0x07,0x1C,0x70,0xC0,0x80,0x80,0x40,0x70,0x1C,0x07,0x01,0x00}, {0x00,0x00,0x00,0x00,0x00,0x04,0xE7,0x7F,0x3F,0x0C,0x04,0x00,0x00,0x00,0x00,0x00}},
       {{0x00,0x00,0x00,0x01,0x07,0x1C,0x70,0x40,0x00,0x00,0x40,0x70,0x1C,0x07,0x01,0x00}, {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x02,0x08,0x00,0x00,0x00,0x00,0x00,0x00,0x00}},
       {{0x00,0x00,0x00,0x01,0x07,0x1C,0x70,0xC0,0x80,0x80,0x40,0x70,0x1C,0x07,0x01,0x00}, {0x00,0x00,0x00,0x00,0x00,0x04,0xE7,0x7F,0x3F,0x0C,0x04,0x00,0x00,0x00,0x00,0x00}},
       {{0x00,0x00,0x00,0x01,0x07,0x1C,0x70,0x40,0x00,0x00,0x40,0x70,0x1C,0x07,0x01,0x00}, {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x02,0x08,0x00,0x00,0x00,0x00,0x00,0x00,0x00}}
    },
    // 15 TOOL_CONFORMAL_COATING
    {
       {{0x00,0x00,0x00,0x00,0x00,0x00,0xCF,0x7F,0xCF,0x8F,0x00,0x00,0x00,0x00,0x00,0x00}, {0x00,0x00,0x24,0x1E,0x66,0x1D,0x13,0x38,0x11,0x07,0x1A,0x66,0x1F,0x22,0x00,0x00}},
       {{0x00,0x00,0x00,0x00,0x00,0x00,0xCF,0x7F,0xCF,0x8F,0x00,0x00,0x00,0x00,0x00,0x00}, {0x00,0x00,0x24,0x1E,0x66,0x1D,0x13,0x38,0x11,0x07,0x1A,0x66,0x1F,0x22,0x00,0x00}},
       {{0x00,0x00,0x00,0x00,0x00,0x00,0xCF,0x7F,0xCF,0x8F,0x00,0x00,0x80,0x00,0x00,0x00}, {0x00,0x00,0x22,0x1F,0x66,0x1D,0x23,0x70,0x21,0x07,0x1A,0x65,0x1B,0x21,0x00,0x00}},
       {{0x00,0x00,0x00,0x00,0x00,0x00,0xCF,0x7F,0xCF,0x8F,0x00,0x00,0x00,0x00,0x00,0x00}, {0x00,0x00,0x24,0x1E,0x66,0x1D,0x13,0x38,0x11,0x07,0x1A,0x66,0x1F,0x22,0x00,0x00}}
    },
    // 16 TOOL_VACUUM_GRIPPER_LG
    {
       {{0x30,0xF8,0xF8,0xB0,0xB0,0xF8,0xF8,0xB0,0xB0,0xF8,0xF8,0xB0,0xB0,0xF8,0xF8,0xB0}, {0x00,0x00,0x00,0x00,0x00,0x70,0x50,0x50,0x5F,0x50,0x50,0x70,0x00,0x00,0x00,0x00}},
       {{0x20,0xF0,0xF0,0xA0,0xA0,0xF0,0xF0,0xA0,0xA0,0xF0,0xF0,0xA0,0xA0,0xF0,0xF0,0xA0}, {0x00,0x00,0x00,0x00,0x00,0x70,0x50,0x50,0x5F,0x50,0x50,0x70,0x00,0x00,0x00,0x00}},
       {{0x30,0xF8,0xF8,0xB0,0xB0,0xF8,0xF8,0xB0,0xB0,0xF8,0xF8,0xB0,0xB0,0xF8,0xF8,0xB0}, {0x00,0x00,0x00,0x00,0x00,0x70,0x50,0x50,0x5F,0x50,0x50,0x70,0x00,0x00,0x00,0x00}},
       {{0x10,0xB8,0xB8,0x90,0x90,0xB8,0xB8,0x90,0x90,0xB8,0xB8,0x90,0x90,0xB8,0xB8,0x90}, {0x00,0x00,0x00,0x00,0x00,0x70,0x50,0x50,0x5F,0x50,0x50,0x70,0x00,0x00,0x00,0x00}}
    },
    // 17 TOOL_FLYING_PROBE
    {
       {{0x00,0x00,0x00,0x00,0x04,0x0E,0x1C,0x38,0x70,0xE0,0xC0,0x80,0x00,0x00,0x00,0x00}, {0x00,0x0C,0x30,0xC0,0x30,0x0C,0x00,0x00,0x00,0x00,0x01,0x03,0x07,0x0F,0x03,0x00}},
       {{0x00,0x00,0x00,0x00,0x04,0x0E,0x1C,0x18,0x30,0x60,0xE0,0xC0,0x80,0x80,0x80,0x00}, {0x00,0x0C,0x30,0xC0,0x30,0x0C,0x00,0x00,0x00,0x00,0x00,0x01,0x03,0x07,0x01,0x00}},
       {{0x00,0x00,0x00,0x00,0x04,0x0E,0x1C,0x18,0x30,0x60,0xE0,0xC0,0x80,0x80,0x80,0x00}, {0x00,0x0C,0x30,0xC0,0x30,0x0C,0x00,0x00,0x00,0x00,0x00,0x01,0x03,0x07,0x01,0x00}},
       {{0x00,0x00,0x00,0x00,0x04,0x0E,0x1C,0x38,0x70,0xE0,0xC0,0x80,0x00,0x00,0x00,0x00}, {0x00,0x0C,0x30,0xC0,0x30,0x0C,0x00,0x00,0x00,0x00,0x01,0x03,0x07,0x0F,0x03,0x00}}
    },
    // 18 TOOL_UV_CURING
    {
       {{0x00,0x00,0x00,0x00,0x00,0x18,0xFC,0xFE,0xFE,0xFC,0x18,0x00,0x00,0x00,0x00,0x00}, {0x00,0x00,0x01,0x12,0x24,0x20,0x01,0x3D,0x01,0x01,0x20,0x24,0x12,0x01,0x00,0x00}},
       {{0x00,0x00,0x00,0x00,0x00,0x18,0xFC,0xFE,0xFE,0xFC,0x18,0x00,0x00,0x00,0x00,0x00}, {0x00,0x01,0x02,0x12,0x24,0x40,0x01,0x7D,0x01,0x01,0x40,0x24,0x12,0x02,0x01,0x00}},
       {{0x00,0x00,0x00,0x00,0x00,0x18,0xFC,0xFE,0xFE,0xFC,0x18,0x00,0x00,0x00,0x00,0x00}, {0x00,0x01,0x02,0x12,0x24,0x40,0x01,0x7D,0x01,0x01,0x40,0x24,0x12,0x02,0x01,0x00}},
       {{0x00,0x00,0x00,0x00,0x00,0x18,0xFC,0xFE,0xFE,0xFC,0x18,0x00,0x00,0x00,0x00,0x00}, {0x00,0x00,0x01,0x12,0x24,0x20,0x01,0x3D,0x01,0x01,0x20,0x24,0x12,0x01,0x00,0x00}}
    },
    // 19 TOOL_HOTAIR_REWORK
    {
       {{0x00,0x00,0x00,0x00,0x60,0x7C,0xFF,0xFF,0xFF,0xFF,0x7C,0x60,0x00,0x00,0x00,0x00}, {0x00,0x00,0x00,0x00,0x00,0x2A,0x14,0x00,0x00,0x14,0x2A,0x00,0x00,0x00,0x00,0x00}},
       {{0x00,0x00,0x00,0x00,0x60,0x7C,0xFF,0xFF,0xFF,0xFF,0x7C,0x60,0x00,0x00,0x00,0x00}, {0x00,0x00,0x00,0x00,0x00,0x14,0x2A,0x00,0x00,0x2A,0x14,0x00,0x00,0x00,0x00,0x00}},
       {{0x00,0x00,0x00,0x00,0x60,0x7C,0xFF,0xFF,0xFF,0xFF,0x7C,0x60,0x00,0x00,0x00,0x00}, {0x00,0x00,0x00,0x00,0x00,0x2A,0x14,0x00,0x00,0x14,0x2A,0x00,0x00,0x00,0x00,0x00}},
       {{0x00,0x00,0x00,0x00,0x60,0x7C,0xFF,0xFF,0xFF,0xFF,0x7C,0x60,0x00,0x00,0x00,0x00}, {0x00,0x00,0x00,0x00,0x00,0x14,0x2A,0x00,0x00,0x2A,0x14,0x00,0x00,0x00,0x00,0x00}}
    },
    // 20 TOOL_PRESSFIT_INSERTER
    {
       {{0x00,0x00,0x00,0x00,0x00,0xFF,0x81,0x81,0x81,0x81,0xFF,0x00,0x00,0x00,0x00,0x00}, {0x00,0x00,0x00,0x00,0x04,0x1C,0x7F,0xFF,0x7F,0x3F,0x0C,0x04,0x00,0x00,0x00,0x00}},
       {{0x00,0x00,0x00,0x00,0x00,0xFF,0x81,0x81,0x81,0x81,0xFF,0x00,0x00,0x00,0x00,0x00}, {0x00,0x00,0x00,0x00,0x08,0x18,0x7F,0xFF,0x7F,0x3F,0x18,0x08,0x00,0x00,0x00,0x00}},
       {{0x00,0x00,0x00,0x00,0x00,0xFF,0x81,0x81,0x81,0x81,0xFF,0x00,0x00,0x00,0x00,0x00}, {0x00,0x00,0x00,0x00,0x10,0x30,0x7F,0xFF,0x7F,0x3F,0x30,0x10,0x00,0x00,0x00,0x00}},
       {{0x00,0x00,0x00,0x00,0x00,0xFF,0x81,0x81,0x81,0x81,0xFF,0x00,0x00,0x00,0x00,0x00}, {0x00,0x00,0x00,0x00,0x08,0x18,0x7F,0xFF,0x7F,0x3F,0x18,0x08,0x00,0x00,0x00,0x00}}
    },
    // 21 TOOL_CRIMPING_ACTUATOR
    {
       {{0x00,0x03,0x0C,0x1C,0x38,0x70,0xE0,0xC0,0xC0,0xE0,0x70,0x38,0x1C,0x0E,0x07,0x00}, {0x00,0x00,0x00,0x00,0x00,0x00,0x73,0x7F,0x0F,0x7F,0x70,0x00,0x00,0x00,0x00,0x00}},
       {{0x00,0x03,0x0C,0x1C,0x38,0x70,0xE0,0xC0,0xC0,0xE0,0x70,0x38,0x1C,0x0E,0x07,0x00}, {0x00,0x00,0x00,0x00,0x00,0x70,0x7F,0x0F,0x03,0x0F,0x7E,0x70,0x00,0x00,0x00,0x00}},
       {{0x00,0x03,0x0C,0x1C,0x38,0x70,0xE0,0xC0,0xC0,0xE0,0x70,0x38,0x1C,0x0E,0x07,0x00}, {0x00,0x00,0x00,0x00,0x70,0x7E,0x0F,0x03,0x03,0x03,0x0E,0x7E,0x70,0x00,0x00,0x00}},
       {{0x00,0x03,0x0C,0x1C,0x38,0x70,0xE0,0xC0,0xC0,0xE0,0x70,0x38,0x1C,0x0E,0x07,0x00}, {0x00,0x00,0x00,0x00,0x00,0x70,0x7F,0x0F,0x03,0x0F,0x7E,0x70,0x00,0x00,0x00,0x00}}
    },
    // 22 TOOL_THERMAL_INSPECTION
    {
       {{0x00,0x00,0xF0,0x10,0x10,0x90,0x50,0x50,0x50,0x5E,0x9E,0x1E,0x1E,0xFE,0x00,0x00}, {0x00,0x00,0x1F,0x10,0x10,0x13,0x14,0x15,0x14,0x14,0x13,0x10,0x10,0x1F,0x00,0x00}},
       {{0x00,0x00,0xF0,0x10,0x10,0x90,0x50,0x50,0x50,0x5E,0x9E,0x1E,0x1E,0xFE,0x00,0x00}, {0x00,0x00,0x1F,0x10,0x10,0x13,0x14,0x14,0x14,0x14,0x13,0x10,0x10,0x1F,0x00,0x00}},
       {{0x00,0x00,0xF0,0x10,0x10,0x90,0x50,0x50,0x50,0x5E,0x9E,0x1E,0x1E,0xFE,0x00,0x00}, {0x00,0x00,0x1F,0x10,0x10,0x13,0x14,0x15,0x14,0x14,0x13,0x10,0x10,0x1F,0x00,0x00}},
       {{0x00,0x00,0xF0,0x10,0x10,0x90,0x50,0x50,0x50,0x5E,0x9E,0x1E,0x1E,0xFE,0x00,0x00}, {0x00,0x00,0x1F,0x10,0x10,0x13,0x14,0x14,0x14,0x14,0x13,0x10,0x10,0x1F,0x00,0x00}}
    },
    // 23 TOOL_PASTE_JETTING
    {
       {{0x00,0x00,0x00,0x07,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x07,0x00,0x00,0x00}, {0x00,0x00,0x00,0x00,0x00,0x30,0x7B,0xFF,0xFF,0x7B,0x30,0x00,0x00,0x00,0x00,0x00}},
       {{0x00,0x00,0x00,0x07,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x07,0x00,0x00,0x00}, {0x00,0x00,0x00,0x00,0x00,0x70,0xFB,0xFB,0xFB,0xFB,0x70,0x00,0x00,0x00,0x00,0x00}},
       {{0x00,0x00,0x00,0x07,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x07,0x00,0x00,0x00}, {0x00,0x00,0x00,0x00,0x00,0x60,0xF3,0xF3,0xF3,0xF3,0x60,0x00,0x00,0x00,0x00,0x00}},
       {{0x00,0x00,0x00,0x07,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x07,0x00,0x00,0x00}, {0x00,0x00,0x00,0x00,0x00,0x60,0xF3,0xF3,0xF3,0xF3,0x60,0x00,0x00,0x00,0x00,0x00}}
    },
    // 24 TOOL_ULTRASONIC_WELDER
    {
       {{0x00,0x00,0x00,0xC0,0xFC,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFC,0xC0,0x00,0x00,0x00}, {0x54,0x54,0x54,0x44,0x00,0x00,0x03,0x03,0x03,0x03,0x00,0x00,0x44,0x54,0x54,0x54}},
       {{0x00,0x00,0x00,0xC0,0xFC,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFC,0xC0,0x00,0x00,0x00}, {0x54,0x54,0x54,0x44,0x00,0x00,0x03,0x03,0x03,0x03,0x00,0x00,0x44,0x54,0x54,0x54}},
       {{0x00,0x00,0x00,0xC0,0xFC,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFC,0xC0,0x00,0x00,0x00}, {0x54,0x54,0x54,0x00,0x00,0x00,0x03,0x03,0x03,0x03,0x00,0x00,0x00,0x54,0x54,0x54}},
       {{0x00,0x00,0x00,0xC0,0xFC,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFC,0xC0,0x00,0x00,0x00}, {0x54,0x54,0x54,0x44,0x00,0x00,0x03,0x03,0x03,0x03,0x00,0x00,0x44,0x54,0x54,0x54}}
    }
};
...
// Regenerated version-text half (columns 74-127) via a real font render
// (Consolas 12pt -> 1-bit -> SSD1306 page-format bytes, see
// docs/tool_image_generator/ sibling tooling for the same general
// approach used on the tool-catalog icons) - the "URTC" wordmark half
// (columns 0-73) is untouched, byte-identical to before. Shows the main
// board application firmware's own MAJOR.MINOR.PATCH (FIRMWARE_VERSION_*
// in firmware_common.h) - NOT auto-synced to that constant at compile
// time (this bitmap has to be regenerated and re-flashed by hand
// whenever the two drift too far apart to stay readable at this size,
// same as the root README's own title badge).
static const uint8_t SplashYellowText[2][128] = {
    {},
    {},
};
...
void OLED_DrawToolIcon(uint8_t tool) {
    // ToolIcons[] has bitmap art for all 25 currently-assigned tools
    // (indices 0-24) - anything above TOOL_ULTRASONIC_WELDER (an
    // unassigned ID, 25-31) falls back to the soldering iron's icon
    // rather than reading out of bounds, same defensive reasoning as
    // before, just with the real boundary now that art exists for the
    // full 13-tool expansion.
    if (tool > TOOL_ULTRASONIC_WELDER) tool = TOOL_SOLDERING_IRON;
    uint8_t buf[16];
...
void Render_ToolScreen(void) {
    ...
    switch(active_tool) {
        ...
        case TOOL_ELECTROMAGNET:      OLED_PrintStr(2, 16, "ELECTROMAGNET  "); break;
        case TOOL_SPOT_WELDER:         OLED_PrintStr(2, 16, "SPOT WELDER    "); break;
        case TOOL_CONFORMAL_COATING:   OLED_PrintStr(2, 16, "CONFORMAL COAT "); break;
        case TOOL_FLYING_PROBE:        OLED_PrintStr(2, 16, "FLYING PROBE   "); break;
        case TOOL_UV_CURING:           OLED_PrintStr(2, 16, "UV CURING      "); break;
        case TOOL_HOTAIR_REWORK:       OLED_PrintStr(2, 16, "HOT AIR REWORK "); break;
        case TOOL_PRESSFIT_INSERTER:   OLED_PrintStr(2, 16, "PRESS-FIT INSRT"); break;
        case TOOL_CRIMPING_ACTUATOR:   OLED_PrintStr(2, 16, "CRIMPING ACTUAT"); break;
        case TOOL_THERMAL_INSPECTION:  OLED_PrintStr(2, 16, "THERMAL INSPECT"); break;
        case TOOL_PASTE_JETTING:       OLED_PrintStr(2, 16, "PASTE JETTING  "); break;
        case TOOL_ULTRASONIC_WELDER:   OLED_PrintStr(2, 16, "ULTRASONIC WELD"); break;
    ...
    switch(active_tool) {
        case TOOL_SOLDERING_IRON: {
            ...
            txt[tp] = 0;
            // Page 4 was the only page left unused on this screen (tool
            // name/divider above at 2-3, temp text/endstop/bar below at
            // 5-7) - a 60s-ish rolling trend of current_temperature, not
            // just its instantaneous value.
            OLED_DrawTempSparkline(4, 5, 85, current_temperature, 0, 450);
