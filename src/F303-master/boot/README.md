...
(`src/F303-master/boot/`) is GPL-3.0. See the repo root README's
"License and Copyright Notices" section for the full breakdown.

This document covers the bootloader (`src/F303-master/boot/`, entry
point `bootloader_main.c`) — the golden-image A/B update system that
runs before the application firmware ever gets control. For the
application firmware itself, see `src/F303-master/README.md`. For the
wire-level CAN protocol (every ID, byte layout, and DLC — including the
`0x7F0`-`0x7FA` range this bootloader owns), see `docs/CANBUS.TXT`.
...
```
0x08000000 ─┬─ Bootloader                30 KB   (this)
0x08007800 ─┼─ Metadata                   2 KB   (version, size, CRC32, HMAC - describes whatever is in the main slot)
0x08008000 ─┼─ Main application slot    112 KB   (the application firmware, what actually runs)
...
this bootloader itself controls:
...
- **BOOT** pulls the `BOOT0` pin high (pin 44, tied to this pushbutton
  ...
  - **`BOOT0` = 0 (default):** boot from main Flash at `0x08000000` —
    which on this board is always this project's own bootloader.
    Everything in this document happens downstream of this path.
  - **`BOOT0` = 1 (held at reset):** boot into the STM32's
    **factory-programmed System Memory bootloader** instead — ST's own
    ...
    supporting recovery over USB DFU or UART. This project's own
    bootloader never runs at all in this mode and knows nothing about
    it.

In short: `0x7F0` gets a healthy *application* to voluntarily jump
into *this project's own* bootloader. The **BOOT** button is a
...
## 7. Files (`src/F303-master/boot/`)

| File | Purpose |
|---|---|
| `bootloader_main.c` | Entry point: clock/CAN/I2C init, `main()`'s own boot sequence and update-listening loop, and every HAL interrupt callback. Matches the application firmware's own `STM32F303CC_main.c` naming for its equivalent entry point. |
...
Plus `STM32F303CCTx_BOOTLOADER.ld` (the linker script - see section 2's
flash layout table) and this README, both alongside the source files
above.

## 8. Build system

`gcc-arm-none-eabi`, producing `.bin`/`.hex`/`.elf`. Every edit bumps
`BOOTLOADER_VERSION_PATCH` (rolling over to `MINOR`+1 after 9) — the
filename never changes, only the compiled-in version constants, since
that's what a version query over CAN actually reports.
