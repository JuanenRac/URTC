...
(`SLAVEAPP.C`, `SLAVEBOOT.C`) is GPL-3.0. The Melexis MLX90640 library
this firmware links against (`melexis/`) is Apache-2.0 - see that
...
This document is the engineering-level reference for `SLAVEAPP.C` (the
application) and `SLAVEBOOT.C` (the bootloader) running on the
...
`src/F303-master/V1.1/README.md`. For pin-by-pin hardware detail, see
...
`src/F303-slave/V1.1/boot/README.md` section 2 for the full table. This
...
| `REG_ENTER_BOOTLOADER` (`0x02`) | write | 4 bytes | Magic payload `0xB0 0x07 0x1D 0x5A` (same constant the main board's own application checks for CAN `0x7F0`) resets into `SLAVEBOOT.C` - see section 5 below |
...
a safe state than the main board's own 12+ tool profiles.
...
this chip's own bootloader (see that README's section 7) - this chip's
own `stm32f303xc.h`/`startup_stm32f303xc.s` are literally the same files
the main board's own STM32F303CC build already uses, since both chips
are the same xB/xC density line.

**The Melexis library is compiled as its own separate object in BOTH
the monolithic and partitioned builds** - never folded into this
project's own single-file `SLAVEAPP.C`, even in the "monolithic" build.
Apache-2.0 requires that code's own copyright notice stay intact;
merging third-party code under a different license into a single blob
would obscure that. "Monolithic" describes this project's own code
here, not vendored dependencies - same principle the main board's own
firmware already applies to the HAL/CMSIS it links against, just made
explicit here since this is the first time this project has vendored a
third-party *application-level* library rather than a hardware-
abstraction one.

---

## 7. Partitioned build (`partitioned/`)

Same dual-maintenance rule as everywhere else in this project - a
change to one form gets mirrored to the other, neither is generated
from or deprecated in favor of the other.
...
| `melexis/MLX90640_API.c` / `.h`, `MLX90640_I2C_Driver.h` | Melexis's own official library (Apache-2.0), unmodified - see section 6's own note on why this stays a separate compilation unit in every build form. Lives at `src/F303-slave/V1.1/melexis/`, one level up from `partitioned/`, shared by both the monolithic and partitioned builds rather than duplicated per form. |

Plus `STM32F303CBTx_SLAVEAPP.ld` (the linker script - see section 1's
flash layout) alongside the monolithic `SLAVEAPP.C` and this README.

**Verified equivalent, not just assumed:** both forms built and
compared - `bss` identical, `.text` differs by well under 1%, same
inlining-across-a-single-translation-unit explanation as the main
board's own equivalent check.
...
| `src/F303-slave/V1.1/boot/README.md` | This chip's own bootloader |
| `src/F303-master/V1.1/README.md` | The main board's own firmware, including its half of the I2C-link relationship this chip is the other end of |
