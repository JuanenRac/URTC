...
| Version | Notes |
|---|---|
| **1.0.3** | Second verification build of the automated bootloader-version bump wired in for 1.0.2 (see that entry) - `build_firmware.bat` run again immediately after, confirming `bump_bootloader_version.py` correctly reads back its own just-written `1.0.2` and produces a second consecutive, correctly-incremented build (`1.0.2 -> 1.0.3`) with 0 warnings/0 failures across all 4 components. Source identical to 1.0.1 - no bootloader behavior change, version-mechanism verification only. |
| **1.0.2** | First bootloader binary built after wiring the ecosystem-wide versioning policy into the real build - same mechanism, same commit, as the main board bootloader's own 1.1.8 entry (see [`../../F303-master/boot/CHANGELOG.md`](../../F303-master/boot/CHANGELOG.md)): `build_firmware.sh`/`.bat` now call `bump_bootloader_version.py` as the first step of compiling THIS bootloader too, bumping `BOOTLOADER_VERSION_PATCH` automatically before compiling, with the same odometer-carry rule into `MINOR`/`MAJOR`. The slave application (`FIRMWARE_VERSION_MAJOR`/`MINOR` in `slave_common.h`) stays static, untouched by this mechanism. Source identical to 1.0.1 otherwise - no bootloader behavior change. |
