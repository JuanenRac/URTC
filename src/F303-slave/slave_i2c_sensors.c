...
// -----------------------------------------------------------------------
// volatile: written from the main loop (MLX_ServiceCapture, deferred
// there specifically so the real capture below never blocks inside an
// ISR - see MLX_TriggerCapture's own comment), read from MLX_GetCaptureStatus
// whenever the link bus (I2C1, this chip's own slave role, served entirely
// from interrupt context) is asked for it - same cross-context reasoning
// already established for i2c_link_last_activity_tick/pulse_remaining_ms
// elsewhere in this project.
static volatile uint8_t mlx90640_capture_status = MLX_CAPTURE_IDLE;
...
// volatile: see mlx90640_capture_status's own comment above - same
// main-loop-writes/ISR-reads reasoning applies here.
static volatile uint8_t mlx90641_capture_status = MLX_CAPTURE_IDLE;
...
// Set from MLX_TriggerCapture (ISR context - see its own comment) and
// consumed from MLX_ServiceCapture (main loop, slave_main.c) - a plain
// binary flag is enough here without a critical section: the only writer
// (the ISR) only ever sets it to 1, and the only reader (the main loop)
// only ever clears it right before actually starting the deferred work,
// so a request that arrives while a previous one is still being serviced
// is never lost, only picked up on the loop iteration after the current
// capture finishes.
static volatile uint8_t mlx_capture_requested = 0;
...
void MLX_TriggerCapture(void) {
    if (mlx_sensor_variant == MLX_VARIANT_90640 || mlx_sensor_variant == MLX_VARIANT_90641) {
        ...
        // TriggerMeasurement + GetFrameData (the real work those
        // functions do) is a whole sequence of blocking I2C2 master
        // transactions - Melexis's own reference timing for a full
        // MLX9064x frame is comfortably capable of taking hundreds of
        // ms, and this project's own firmware_mlx90640_app.c (the main
        // board's direct-connection driver) already documents a real,
        // measured >500ms worst case for the same underlying capture.
        // Blocking that long INSIDE this ISR callback left I2C1 deaf to
        // the main board for the whole duration (no other register
        // write/read on this link could be serviced until the callback
        // returned) and, worse, starved this chip's own
        // HAL_IWDG_Refresh() (only ever reached from the main loop's own
        // while(1), see slave_main.c) for the same stretch - long enough
        // to risk tripping the ~0.8s watchdog on an ordinary capture, not
        // just an unusually slow one. Setting BUSY immediately below (so
        // a host that polls status right after triggering sees the
        // correct thing with no delay) and flagging the real work for
        // MLX_ServiceCapture keeps this ISR callback itself fast -
        // exactly the same "start something async, poll for completion
        // separately" shape MLX90642_DoTriggerCapture/
        // MLX90642_DoGetCaptureStatus below already use for that sensor.
        if (mlx_sensor_variant == MLX_VARIANT_90640) {
            mlx90640_capture_status = MLX_CAPTURE_BUSY;
        } else {
            mlx90641_capture_status = MLX_CAPTURE_BUSY;
        }
        mlx_capture_requested = 1;
    } else if (mlx_sensor_variant == MLX_VARIANT_90642) {
...
// Called once per main-loop iteration (slave_main.c) - a cheap no-op the
// overwhelming majority of the time, since a real capture is only ever
// requested when the link bus actually asks for one. Runs the same
// MLX90640_DoTriggerCapture/MLX90641_DoTriggerCapture blocking work
// MLX_TriggerCapture used to run inline, deferred here specifically to
// keep it out of I2C1's own ISR context - see MLX_TriggerCapture's own
// comment for the full reasoning.
void MLX_ServiceCapture(void) {
    if (!mlx_capture_requested) return;
    mlx_capture_requested = 0;
    if (mlx_sensor_variant == MLX_VARIANT_90640) {
