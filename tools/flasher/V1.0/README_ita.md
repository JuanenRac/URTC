...
`tools/flasher/V1.0/`**, proprio accanto a `urtc_flasher.py`:
...
Questo è deliberato: mantenere `firmware/` dentro `tools/flasher/V1.0/`
invece che nella radice del repository significa che l'intera cartella
`tools/flasher/V1.0/` è autonoma. Se vuoi solo flashare una scheda - su
un PC di officina, da una chiavetta USB, ovunque - puoi copiare
`tools/flasher/V1.0/` da sola senza nient'altro dal repository, e
funziona comunque.
...
`tools/flasher/V1.0/logs/` (`urtc_flasher_YYYYMMDD_HHMMSS.log`),
...
parametri di `src/F303-master/V1.0/README.md`) - questa casella esiste
