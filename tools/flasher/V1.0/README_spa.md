...
**Versión:** 1.0 (la versión de esta herramienta - se muestra en el banner
...
`tools/flasher/V1.0/`**, justo al lado de `urtc_flasher.py`:
...
Esto es deliberado: mantener `firmware/` dentro de `tools/flasher/V1.0/`
en vez de en la raíz del repositorio significa que toda la carpeta
`tools/flasher/V1.0/` es autocontenida. Si solo quieres flashear una
placa — en un PC de taller, desde una memoria USB, donde sea — puedes
copiar `tools/flasher/V1.0/` por sí sola sin nada más del repositorio, y
sigue funcionando.
...
- **`v1.0 (application, HardwareID 0x0303CC01)`** - caso normal,
  aplicación en ejecución, todo coincide.
- **`Bootloader running, no valid firmware currently installed,
  bootloader v1.0.0`** - la placa está atascada en el bootloader sin
...
en `tools/flasher/V1.0/logs/` (`urtc_flasher_YYYYMMDD_HHMMSS.log`),
...
persistencia de parámetros de `src/F303-master/V1.0/README.md`) - esta
