...
This tool expects a `firmware/` folder **inside `tools/flasher/V1.0/`**, right next to
`urtc_flasher.py`:
...
This is deliberate: keeping `firmware/` inside `tools/flasher/V1.0/` instead of at
the repo root means the whole `tools/flasher/V1.0/` folder is self-contained. If
you just want to flash a board — on a shop floor PC, from a USB stick,
wherever — you can copy `tools/flasher/V1.0/` on its own with nothing else from
...
Every session also writes a timestamped log file to `tools/flasher/V1.0/logs/`
...
boot (see `src/F303-master/V1.0/README.md`'s parameter-persistence section) - this
