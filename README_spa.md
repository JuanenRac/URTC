...
**Política de versionado:** los 2 números de versión del firmware de aplicación (`FIRMWARE_VERSION_MAJOR`/`MINOR`, placa principal y esclavo de expansión) son **estáticos** - solo cambian cuando un humano los edita a mano. Los 2 números de versión de bootloader (`BOOTLOADER_VERSION_MAJOR`/`MINOR`/`PATCH`) son **incrementales** - cada compilación real sube `PATCH` en 1 automáticamente (`bump_bootloader_version.py` en la raíz del repositorio, invocado por `build_firmware.sh`/`.bat` justo antes de compilar cada bootloader), con acarreo hacia `MINOR` (y luego `MAJOR`) cuando `PATCH` superaría 9, la misma regla en base 10 de "cuentakilómetros" que usa un odómetro real - p. ej. `1.1.7` → `1.1.8` → `1.1.9` → `1.2.0`, nunca `1.1.10`. Ver [`CHANGELOG.md`](CHANGELOG.md) en la raíz del repositorio para el estado actual de los 4 componentes de un vistazo, y [`VERSION_CHECKLIST.txt`](VERSION_CHECKLIST.txt) para la mecánica completa por pista.
